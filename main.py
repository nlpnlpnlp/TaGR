...
from tag_rnp_model import StudentTagRnpModel,TeacherEncoder
...
def cross_merge(rationale_text, suprious_text, noise_text):
    ...
    while i < len(rationale_sents) or j < len(suprious_sents) or k < len(noise_sents):
        if i < len(rationale_sents):
            tokens = rationale_sents[i].split()  # 按空格分token
...
def texts_to_tensor(texts,rationales, word2idx, max_len,labels): # max_len可能会截断证据
    n = len(texts)
    ...
    for i, t in enumerate(texts):
        ...
        suprious_text = all_text.replace(rationale_text, "")  # 替换为空字符串 noise_text = all_text - rationale_text
        ...
        for j, w in enumerate(toks):
            ...
            # inputs[i, j] = vocab.get(w, 0) #vocab["<UNK>"]
            # masks[i, j] = 1.

        # construct rationale
        for zs in rationale_pos: #[(0, 15), (17, 35), (52, 64), (81, 98), (115, 137), (154, 174)]
            start = zs[0]
            end = zs[1]
            if start >= max_len:
                continue
        ...
        if(i==2025):
            rationale_pos0 = rationale_pos
            rationale_pos1 = rationale_masks[i]

            # [Case Based, Genetic Algorithms, Neural Networks, Probabilistic Methods, Reinforcement Learning, Rule Learning, Theory]
            print("rationale_text",rationale_text)
            print("suprious_text",suprious_text)
            print("noise_text",noise_text)
            print("Label",label)
            print("merged_tokens"," ".join(merged_tokens))
            print("rationale_pos0",rationale_pos0)
            print("rationale_pos1"," ".join([str(int(item)) for item in rationale_pos1]))


    return torch.from_numpy(inputs), torch.from_numpy(masks),torch.from_numpy(rationale_masks)

def texts_to_tensor_teacher(texts,rationales, word2idx, max_len,labels): # max_len可能会截断证据
    n = len(texts)
    inputs = np.zeros((n, max_len), dtype=np.int64)
    masks = np.zeros((n, max_len), dtype=np.float32)
...
def texts_to_tensor_train(texts,data, word2idx, max_len):
    n = len(data.train_mask)
    inputs = np.zeros((n, max_len), dtype=np.int64)
    masks = np.zeros((n, max_len), dtype=np.float32)
    ...
    train_inputs = inputs[data.train_mask]
    train_masks = masks[data.train_mask]
    return torch.from_numpy(train_inputs),torch.from_numpy(train_masks)
...
def evaluate(model, data, inputs, masks,rationale_masks, device):
    ...
    with torch.no_grad():
        z, logits = model(inputs, masks, data.edge_index)
...
def evaluate_for(model, data, inputs, masks,rationale_masks, device):
    model.eval()
    ...
    results_list = []
    with torch.no_grad():
        z, logits,z_list = model(inputs, masks, data.edge_index,is_evaluating=True)
        pred = logits.argmax(dim=1).cpu()
        y = data.y.cpu()
...
def evaluate_(model, data, inputs, masks,rationale_masks, device):
    model.eval()
    ...
    with torch.no_grad():
        ...
        for z in new_list:
            ...
            train_micro_precision = train_num_true_pos_ / train_num_predicted_pos_
            train_micro_recall = train_num_true_pos_ / train_num_real_pos_
            train_micro_f1 = 2 * (train_micro_precision * train_micro_recall) / (train_micro_precision + train_micro_recall)
            train_sparsity = train_num_predicted_pos_ / train_num_words

            val_micro_precision = val_num_true_pos_ / val_num_predicted_pos_
            val_micro_recall = val_num_true_pos_ / val_num_real_pos_
            val_micro_f1 = 2 * (val_micro_precision * val_micro_recall) / (val_micro_precision + val_micro_recall)
            val_sparsity = val_num_predicted_pos_ / val_num_words

            test_micro_precision = test_num_true_pos_ / test_num_predicted_pos_
            test_micro_recall = test_num_true_pos_ / test_num_real_pos_
            test_micro_f1 = 2 * (test_micro_precision * test_micro_recall) / (test_micro_precision + test_micro_recall)
            test_sparsity = test_num_predicted_pos_ / test_num_words
...
def main(args):
    ...
    if(args.data_type=="cora"):
        data, texts,rationales = get_raw_text_cora(use_text=True, seed=args.seed)
    elif(args.data_type=="pubmed"):
        data, texts,rationales = get_raw_text_pubmed_subgraph_aligned(use_text=True, seed=args.seed)
    elif(args.data_type=="products"):
        data, texts,rationales = get_raw_text_products_subgraph_aligned(use_text=True, seed=args.seed)
    elif(args.data_type=="arxiv_2023"):
        data, texts,rationales = get_raw_text_arxiv_2023_subgraph_aligned(use_text=True, seed=args.seed)
    else:
        pass
        sys.exit(1)
    ...
    # Teacher
    teacher_inputs, teacher_masks = texts_to_tensor_teacher(texts,rationales, word2idx, args.max_len,data.y)
    ...
    model = StudentTagRnpModel(args_model)
    model = model.to(device)



    # discrim_para=[{'params': model.gnn_layers.parameters(), 'lr':args.lr}]
    ...
    # distribution_optimizer = torch.optim.Adam(model.parameters(), lr=args.lr)
    ...
    # #     {'params': teacher_model.gnn_layers.parameters(), 'lr':args.lr},
    # # ]
    # teacher_optimizer = torch.optim.Adam(teacher_model.parameters(), lr=args.lr)
    ...
    for epoch in tqdm(range(1, args.epochs + 1)):
        ...
        z, student_logits,output_list = model(inputs, masks, data.edge_index) 
        ...
        # with torch.no_grad(): #只学习了伪类别
        #     teacher_logits = teacher_model(inputs, masks, data.edge_index).detach()
        #     teacher_soft_labels = teacher_logits.argmax(dim=1).detach()
        #     distribution_loss = args.cls_lambda * F.nll_loss(student_logits[data.train_mask], teacher_soft_labels[data.train_mask])
        ...
        teacher_logits = teacher_model(inputs, masks, data.edge_index).detach()
        # teacher_soft_labels = teacher_logits.argmax(dim=1).detach()
        T = args.temperature  # e.g., 2.0
            
        teacher_probs = F.softmax(teacher_logits[data.train_mask] / T, dim=1)  # teacher 分布（soft）
        student_log_probs = F.log_softmax(student_logits[data.train_mask] / T, dim=1)  # student 分布（log softmax）
        # KL 散度作为蒸馏损失
        ...
        student_probs = F.softmax(student_logits[data.train_mask] / T, dim=1)  # teacher 分布（soft）
        teacher_log_probs = F.log_softmax(teacher_logits[data.train_mask] / T, dim=1)  # student 分布（log softmax）
        # KL 散度作为蒸馏损失
        ...
        distillation_loss = 0.5*(distillation_loss_1 + distillation_loss_2)

        layer_distribution_loss = 0.0
        for cur_student_logits in output_list:
            cur_student_log_probs = F.log_softmax(cur_student_logits[data.train_mask] / T, dim=1)  # student 分布（log softmax）
            ...
            layer_distribution_loss = layer_distribution_loss + 0.5*(cur_layer_distillation_loss_1+cur_layer_distillation_loss_2)
        ...
        print(distribution_loss.shape, distribution_loss.requires_grad)
        ...
        if epoch % 1 == 0 or epoch == 1:
            # acc_train, acc_val, acc_test, (train_sparsity,train_micro_precision, train_micro_recall,train_micro_f1),\
            # (val_sparsity,val_micro_precision, val_micro_recall,val_micro_f1),(test_sparsity,test_micro_precision, test_micro_recall,test_micro_f1) = \
            # evaluate(model, data, inputs, masks,rationale_masks, device)


            # print(f"Epoch #{epoch:03d}:")
            # print(f"Loss {loss.item():.4f} CLS {cls_loss.item():.4f} Spar {sparsity_loss.item():.4f} Cont {continuity_loss.item():.4f}")
            # print(f"Train {acc_train:.4f} Val {acc_val:.4f} Test {acc_test:.4f}")
    ...
    for epoch in tqdm(range(1, args.epochs + 1)):
        ...
        if (args.visual_interval and ((epoch + 1) % args.visual_interval in [0, 1]) and epoch != 0):
            ...
            import random
            sample_id = 328 #random.randint(0, batch_size - 1)
            labels = data.y.cpu()
        ...
        if epoch % 1 == 0 or epoch == 1:
            ...
            current_epoch_result = {
                ...
                "vali_s":val_sparsity,
                "vali_delta_s": torch.abs(val_sparsity - args.sparsity_percentage).item(),
                ...
                "test_delta_s": torch.abs(test_sparsity - args.sparsity_percentage).item(),
            }
