...
def get_raw_text_cora(use_text=False, seed=0,use_gpt_explanation=False):
    ...
    path = './gpt_noise_responses/cora/cora_text_data_all_noise_gpt-3.5-turbo-1106_all.json'
    text = []
    rationale = []
...
if __name__=='__main__':
    print(111)
    data, text_list = get_raw_text_cora_generate_data(use_text=True, seed=0)
    n = data.y.shape[0]

    all_list = []
    for i in range(n):
        text_example = text_list[i]
        text_example['label'] = np.array(data.y[i]).tolist()
        all_list.append(text_example)
 
    with open("/home/cfn/work/code_baselines_v100/SETAG/TAPE/dataset/cora_sys_text/cora_text_data_all.json", encoding='utf-8', mode='w') as file:
        file.write(json.dumps(all_list,indent=4))
